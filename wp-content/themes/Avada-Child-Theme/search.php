...
<?php get_header(); ?>
<section id="content" class="search-result-page" <?php echo esc_attr(apply_filters('content_class', '' ) ); ?>style="<?php echo esc_attr(apply_filters('content_style', '' ) ); ?>">
	...
	<?php else : ?>
        ...
        <!-- custome code get filter search form -->
        <?php
            $pageSearchForm = get_posts([
            ...
            if ( $pageSearchForm )
            {
                echo do_shortcode($pageSearchForm[0]->post_content);
            }
        ?>
