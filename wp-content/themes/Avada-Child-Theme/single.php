...
<section id="content" style="<?php esc_attr_e( apply_filters( 'awb_content_tag_style', '' ) ); ?>">
	...
	<?php while ( have_posts() ) : ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class( 'post' ); ?>>
			...
			<?php if ( ! post_password_required( $post->ID ) ) : ?>
				<?php if ( Avada()->settings->get( 'featured_images_single' ) ) : ?>
					...
					<?php if ( 0 < avada_number_of_featured_images() || $video ) : ?>
						...
						<?php echo avada_render_post_metadata( 'single' ); // adding category on top of the image ?>
						<ul class="slides">
								<?php if ( $video ) : ?>
									<li>
			...
			<div id="post-custom-container" class="news__content">
			...
			<div class="post-content">
				...
				<?php if ( ! post_password_required( $post->ID ) ) : ?>
					...
					<?php if ( ( Avada()->settings->get( 'author_info' ) && 'no' !== $author_info ) || ( ! Avada()->settings->get( 'author_info' ) && 'yes' === $author_info ) ) : ?>
						<section class="about-author">
							...
							<?php $title = sprintf( __( ' %s', 'Avada' ), ob_get_clean() ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited ?>
					...
					<?php endif; ?>
							...
							<!--Custome widget area start-->
							<?php if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar("single-page-bottom") ) : ?>
							<?php endif;?>
							<!--Custome widget area end-->
				...
				<!-- newsletter block -->
				<div class="news__newsletter multi-step-form">
					<form id="pardot-form" method="post" action="https://go.intelematics.com/l/715203/2021-01-28/wpx1x" class="form">
						<div id="pardot-form-content"></div>

						<div class="pardot-buttons-row">
